 ...
 * For: If no pages avaliable than if scroll redirect will be to /blabla/undefined - that's not true
 */
	...
	$.fn.paginator = function (s){			
		...
		function initEvents (){
			...
			function executeLink(el){
				// @editor Azat Khuzhin
				// If no pages avaliable than if scroll redirect will be to /blabla/undefined - that's not true
				if (el.length <= 0) {
					return false;
				}
			...
			$(html.holder).bind('mousewheel', function (e){
				selector = (e.delta > 0) ? (options.returnOrder ? '.right.top a' : '.left.top a')
										 : (options.returnOrder ? '.left.top a'  : '.right.top a');
